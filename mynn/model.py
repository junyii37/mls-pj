...
class Model:
    ...
    def save_model(self, file_path):
        ...
        for layer in self.layers:
            ...
            if hasattr(layer, 'set_params') and isinstance(layer, nn.BasicBlock):
                # 递归保存子层参数
                sub_params = []
                for sublayer in [layer.conv1, layer.bn1, layer.conv2, layer.bn2,
                                 layer.short_conv, layer.short_bn]:
                    if sublayer is not None and hasattr(sublayer, 'params'):
                        sub_params.append({k: v.get() for k, v in sublayer.params.items()})
                    else:
                        sub_params.append(None)
                model_info['params'].append(sub_params)
            else:
